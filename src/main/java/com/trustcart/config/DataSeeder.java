...
public class DataSeeder {
    ...
    CommandLineRunner seed(BuyerAccountRepository buyers, SellerRepository sellers, ProductRepository products, DiscountCodeRepository discounts, GiftRegistryRepository registries, GiftRegistryItemRepository registryItems) {
        return args -> {
            ...
            seedEssentialsBundles(products, daily, pantry, eco, local);
    ...
    private void seedEssentialsBundles(ProductRepository repo, Seller daily, Seller pantry, Seller eco, Seller local) {
        baseIfMissing(repo,"Pantry Starter Bundle",ProductCategory.GROCERIES,999,pantry,"/img/products/groceries.svg",true,false,"Curated starter set with rice, cooking oil, canned goods, condiments, sugar, and coffee for practical household replenishment.","Essential bundle for fewer separate deliveries");
        baseIfMissing(repo,"Hygiene Kit Bundle",ProductCategory.BEAUTY_PERSONAL_CARE,499,daily,"/img/products/bath-soap.svg",true,false,"Daily hygiene bundle with soap, toothpaste, shampoo, sanitary pads, hand soap, and basic care items.","Grouped household hygiene essentials");
        baseIfMissing(repo,"New Baby Essentials Kit",ProductCategory.BABY_KIDS,899,daily,"/img/products/baby.svg",true,false,"Starter baby-care kit with baby wipes, gentle soap, cotton basics, and newborn household essentials.","Family essentials bundle");
        baseIfMissing(repo,"Cleaning Kit Bundle",ProductCategory.HOME_LIVING,599,eco,"/img/products/home-cleaning.svg",true,false,"Home cleaning bundle with detergent, dishwashing refill, vinegar, baking soda, sponge set, and trash bags.","Low-waste home cleaning bundle");
        baseIfMissing(repo,"Office Snack Kit",ProductCategory.GROCERIES,699,local,"/img/products/coffee.svg",true,false,"Office pantry and snack kit with coffee, oats, crackers, local snacks, and refill-friendly drinks.","Office daily-needs bundle");
        baseIfMissing(repo,"Pet Care Monthly Pack",ProductCategory.PET_SUPPLIES,649,eco,"/img/products/pet.svg",true,false,"Monthly pet-care basics with shampoo, litter, treats, and routine supplies from verified sellers.","Autoship-ready pet essentials");
        baseIfMissing(repo,"Baby Wipes and Diaper Monthly Pack",ProductCategory.BABY_KIDS,799,daily,"/img/products/baby.svg",true,false,"Monthly baby-care replenishment pack for wipes, diapers, and routine family essentials.","Autoship-ready baby essentials");
    }
    ...
    private void normalizeExistingCatalog(ProductRepository repo) {
        for (Product product : repo.findAll()) {
            ...
            if (category == ProductCategory.MOBILE_ACCESSORIES) {
                ...
                changed = true;
            } else if (category == ProductCategory.PREPARED_FOODS) {
                product.setCategory(ProductCategory.PREPARED_FOOD);
                changed = true;
            ...
            if (product.getCategory() != null && product.getCategory().isPreparedFood()) {
    ...
    private String imageFor(String name, ProductCategory category) {
        ...
        if ((category != null && category.isPreparedFood()) || n.contains("adobo") || n.contains("pancit") || n.contains("kakanin") || n.contains("meal")) return "/img/products/meal-adobo.svg";
