...
public class SellerController {
    ...
    private boolean isAcceptableProductImage(String image) {
        if (image == null || image.isBlank()) return false;
        String value = image.toLowerCase(Locale.ROOT);
        return !(value.contains("unsplash.com") || value.contains("placeholder") || value.contains("broken") || value.contains("no-image"));
    }

    private String cleanProductImageFor(String name, ProductCategory category) {
        String n = name == null ? "" : name.toLowerCase(Locale.ROOT);
        if ((category != null && category.isPreparedFood()) || n.contains("meal") || n.contains("adobo") || n.contains("pancit")) return "/img/products/meal-adobo.svg";
        if (n.contains("baby")) return "/img/products/baby.svg";
        if (n.contains("pet") || n.contains("dog") || n.contains("cat")) return "/img/products/pet.svg";
        if (n.contains("water")) return "/img/products/water-refill.svg";
        if (n.contains("rice")) return "/img/products/rice.svg";
        if (n.contains("soap") || n.contains("hygiene") || n.contains("shampoo") || n.contains("sanitary")) return "/img/products/bath-soap.svg";
        if (n.contains("clean") || n.contains("detergent") || n.contains("tissue") || n.contains("trash")) return "/img/products/home-cleaning.svg";
        if (category == ProductCategory.SUSTAINABLE_PRODUCTS) return "/img/products/organic-vegetables.svg";
        ...
        if (category == ProductCategory.BABY_KIDS) return "/img/products/baby.svg";
        if (category == ProductCategory.PET_SUPPLIES) return "/img/products/pet.svg";
        if (category == ProductCategory.SCHOOL_OFFICE) return "/img/products/office.svg";
    ...
    public String createProduct(@RequestParam String name, @RequestParam String description,
        ...
        p.setPrice(price);
        p.setStock(stock);
        String submittedImage = firstNonBlank(saveUpload(productImage), imageUrl);
        boolean replacedImage = !isAcceptableProductImage(submittedImage);
        p.setImageUrl(replacedImage ? cleanProductImageFor(name, category) : submittedImage);
        ...
        p.setReviewSummary(replacedImage
                ? "Seller-uploaded product. TrustCart replaced a missing or low-quality image with a clean category image before approval."
                : "Seller-uploaded product. Buyer protection applies after checkout.");
        ...
        if (category != null && category.isPreparedFood()) {
            seller.setServiceRadiusKm(Math.min(5, seller.getServiceRadiusKm() == null ? 5 : seller.getServiceRadiusKm()));
            sellerRepository.save(seller);
            p.setSubscriptionEligible(false);
            p.setStockStatus("Pre-order or prepared today · Nearby only");
            p.setEstimatedDelivery("Nearby only · Same-day or scheduled pre-order");
            p.setFulfillmentNote("Seller-prepared food; close-range delivery only. No long-distance shipping.");
            p.setWarrantyPolicy("Food quality issue must be reported on delivery day with photo proof through TrustCart support.");
        }
