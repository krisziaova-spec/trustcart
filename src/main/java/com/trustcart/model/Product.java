...
public class Product {
    ...
    public boolean isPreparedFoodListing() {
        return category != null && category.isPreparedFood();
    }

    public boolean isEssentialsBundle() {
        String n = name == null ? "" : name.toLowerCase();
        return n.contains("starter") || n.contains("kit") || n.contains("bundle") || n.contains("monthly pack") || n.contains("value pack");
    }

    public boolean isOrganicCornerItem() {
        String n = name == null ? "" : name.toLowerCase();
        return category == ProductCategory.SUSTAINABLE_PRODUCTS || n.contains("organic") || n.contains("refill") || n.contains("eco") || n.contains("coconut") || n.contains("low-waste");
    }

    public String getFoodSafetyBadge() {
        if (!isPreparedFoodListing()) return null;
        String statusText = stockStatus == null ? "" : stockStatus.toLowerCase();
        if (statusText.contains("pre-order")) return "Pre-Order Only";
        if (statusText.contains("same-day")) return "Same-Day Only";
        if (statusText.contains("prepared")) return "Prepared Today";
        return "Nearby Only";
    }

    public String getImageQualityLabel() {
