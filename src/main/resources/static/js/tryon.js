...
function selectGarment(btn) {
  ...
  if (asset.includes('pants') || asset.includes('skirt')) {
...
function showGender(gender) {
  ...
  const activeButton = Array.from(document.querySelectorAll('.tab')).find(t => t.textContent.toUpperCase().includes(gender === 'MEN' ? 'MEN' : 'WOMEN'));
  activeButton?.classList.add('active');
  document.querySelectorAll('.tryon-item').forEach(item => {
    item.style.display = item.dataset.gender === gender ? 'grid' : 'none';
  });
}
